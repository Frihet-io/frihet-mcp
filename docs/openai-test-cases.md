...
The ChatGPT connector exposes 33 reviewed business operations plus the 3 read-
only discovery operations `list_tool_groups`, `search_tools`, and
`describe_tool` (36 total). It exposes zero MCP prompts and zero MCP resources.
...
that payroll/HR execution, banking records, and regulated e-invoice generation
or submission do not invoke Frihet.

Load-bearing automated coverage verifies:

- the exact 36 tool names, descriptions, schemas, annotations, and OAuth metadata;
...
- grouped discovery confinement to the five reviewed domains;
